...
# One row per exclusion rule, so a rule that stops firing fails here rather
# than quietly changing the analysis sample.
...
# The model's `grouptreatment` coefficient, and every table that labels it,
# depends on control being the reference level.
...
# Pins the checked-in fixture: 243 rows in, 10 excluded. Update deliberately
# if dummy_raw.csv is regenerated.
